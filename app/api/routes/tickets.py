...
@router.post("/tickets/", response_model=tickets.TicketResponse)
def create_ticket(ticket: tickets.TicketCreate, db: Session = Depends(get_db)):
  db_ticket = crud.create_ticket(db=db, title=ticket.title, project_id=ticket.project_id, description=ticket.description, status=ticket.status, priority=ticket.priority)
  if db_ticket is None:
    raise HTTPException(status_code=400, detail="Ticket could not be created")
  return db_ticket

@router.get("/tickets/{ticket_id}", response_model=tickets.TicketResponse)
...
@router.delete("/tickets/{ticket_id}", response_model=tickets.TicketResponse)
